...
class UserService {
  static Future<Result<User>> createUserRequest(String liquidAddress) async {
    try {
      final response = await http.post(
        Uri.parse('https://8fbe-84-90-103-7.ngrok-free.app/users'),
        body: jsonEncode({
          'user': {
  ...
  static Future<Result<List<Transfer>>> getUserTransactions(String pixPaymentCode, String auth) async {
    try {
      final uri = Uri.parse('https://8fbe-84-90-103-7.ngrok-free.app/users/user_transfers')
  ...
  static Future<Result<String>> getAmountTransferred(String pixPaymentCode, String auth) async {
    try {
      final uri = Uri.parse('https://8fbe-84-90-103-7.ngrok-free.app/users/amount_transfered_by_day')
          .replace(queryParameters: {
        'payment_id': pixPaymentCode,
      });
  ...
  static Future<Result<String>> updateLiquidAddress(String liquidAddress, String auth) async {
    try {
      final response = await http.patch(
        Uri.parse('https://8fbe-84-90-103-7.ngrok-free.app/users/update_liquid_address'),
        body: jsonEncode({
          'user': {
            'liquid_address': liquidAddress,
          }
  ...
  static Future<Result<User>> showUser(String auth) async {
    try {
      final response = await http.get(
        Uri.parse('https://8fbe-84-90-103-7.ngrok-free.app/users/show_user'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': auth,
        },
